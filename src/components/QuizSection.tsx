...
const questions: Question[] = [
  {
    ...
    correctAnswer: 2,
    explanation: 'Hash tables provide O(1) average time complexity for search operations due to direct indexing via hash function.',
  },
  {
    id: 2,
    ...
    correctAnswer: 2,
    explanation: 'Chaining (Separate Chaining) uses linked lists to store multiple entries that hash to the same index.',
  },
  {
    id: 3,
    ...
    correctAnswer: 1,
    explanation: 'When load factor exceeds the threshold (typically 0.75), rehashing occurs where the table size is increased and all entries are redistributed.',
  },
  {
    id: 4,
    ...
    correctAnswer: 2,
    explanation: 'A good hash function should distribute keys uniformly. Always returning the same value would cause all keys to hash to the same index, defeating the purpose.',
  },
  {
    id: 5,
    ...
    correctAnswer: 2,
    explanation: 'Prime numbers are best for table size in the division method as they help reduce clustering and provide better distribution of hash values.',
  },
];
      ...
      <section id="quiz">
        <div className="max-w-3xl mx-auto">
          <motion.div
            ...
            className="p-8 rounded-2xl border border-white/10 bg-white/5 text-center relative overflow-hidden"
            ...
            <div className="relative z-10">
              ...
              <div className="mb-8">
                <div className="text-6xl mb-2">
                  ...
                  <span className="text-[#A0A0A0]">/{questions.length}</span>
                </div>
                <p className="text-[#4CC9F0]">{percentage.toFixed(0)}% Correct</p>
              </div>

              <div className="w-full max-w-md mx-auto mb-8 h-3 bg-white/10 rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
  ...
  return (
    <section id="quiz">
      <div className="max-w-3xl mx-auto">
        ...
        >
          <h2 className="text-white mb-4">Test Your Knowledge</h2>
                ...
                {question.options.map((option, index) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full p-4 rounded-lg border transition-all duration-300 text-left flex items-center justify-between group"
                      style={{
                    ...
                    >
                      <span
                        ...
                        style={{
                          color: showCorrect || showIncorrect ? '#fff' : '#A0A0A0',
                        }}
                      >
                        {option}
                      </span>
                      {showCorrect && <CheckCircle2 className="w-5 h-5 text-[#00FFB2]" />}
                      {showIncorrect && <XCircle className="w-5 h-5 text-[#FF4D6D]" />}
                    </button>
                  );
                })}
              </div>
              ...
              <AnimatePresence>
                {showResult && (
                  <motion.div
                    ...
                    className="mb-6 p-4 rounded-lg bg-[#4CC9F0]/10 border border-[#4CC9F0]/30"
                  >
                    <p className="text-[#E6E6E6] text-sm">{question.explanation}</p>
                  </motion.div>
                )}
              </AnimatePresence>
